...
# Verified example for Helios. Adapt values after checking hpc-grants and hpc-fs.
: "${PLG_GROUP:?Set PLG_GROUP to the writable group shown by hpc-fs}"
GROUP="$PLG_GROUP"
PROJECT_NAME="${PROJECT_NAME:-plgrid-gh200-smoke}"
...
ensure_link "$PROJECT/.local" "$HEAVY/local-arm64"
ensure_link "$PROJECT/.cache" "$HEAVY/cache"
ensure_link "$PROJECT/.venv" "$HEAVY/envs/gh200"
