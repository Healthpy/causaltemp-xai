...
Most counterfactual explanation methods were designed for i.i.d. tabular data.
When applied to multivariate time-series generated by a structural causal model
(SCM), these methods frequently produce explanations that are *causally
unfaithful*: they alter variables at time steps that should remain unchanged
(retroactive edits), or they fail to propagate an intervention forward through
the causal graph correctly.
