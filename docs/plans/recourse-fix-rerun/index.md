...
**Goal**: Fix the NoiselessSCMRecourse/PearlSCMRecourse delta-collapse, make the shipped metric tables
...
The run-1 artifacts keep their original `CARLA` and `PearlCARLA` labels. In this plan those
historical labels map to `NoiselessSCMRecourse` and `PearlSCMRecourse`; all run-2 files, registry
keys, and tables use only the new names.

### The primary defect: one bug, not two

`docs/full_run_2026-08-18.md` section 7.1 reports two separate failures on `full_nl`:
PearlSCMRecourse fully degenerate (`frac_degenerate=1.00`, `n_cf_faith_scorable=0`,
`proximity_l1~=7.5e-05`) and NoiselessSCMRecourse fully vacuous (`validity=0.000`, `frac_vacuous=1.00`,
...
(`causaltemp_xai/methods/counterfactual/scm_recourse.py`, in both candidate loops). That objective has a
...
|---|---|---|---|---|
| NoiselessSCMRecourse | 0.5 | 1.0641593e-05 | 1.0641516e-05 | 1.0641593e-05 |
| PearlSCMRecourse | 0.1 | 3.1715e-05 | 1.5855e-04 | 1.5858e-04 |
...
|---|---|---|---|---|
| `full_nl` | NoiselessSCMRecourse | 6.4e-08 | 26 (= t0+1) | `frac_vacuous=1.00` |
| `full_nl` | PearlSCMRecourse | 7.5e-08 | 99 (= T-1) | `frac_degenerate=1.00` |
| `smoke_nl` | NoiselessSCMRecourse | 3.3e-05 | 9 (= t0+1) | `frac_vacuous=1.00` |
| `smoke_nl` | PearlSCMRecourse | 1.7e-04 | 29 (= T-1) | `frac_degenerate=1.00` |
| `full` (linear) | NoiselessSCMRecourse | 5.4e-03 | 25 (= t0) | "works", `validity=1.000` |
| `full` (linear) | PearlSCMRecourse | 2.9e-02 | 25 (= t0) | `validity=0.000` (section 7.4) |
...
NoiselessSCMRecourse's `validity = 1.000` on the linear `full` tier is **100% noise-deletion** by the
noiseless rollout (`scm_recourse.py`), requiring no intervention whatsoever. It escapes the
vacuity flag only because its delta (5.4e-3) happens to land 5x above the 1e-3 tolerance.
The nonlinear mechanism did not break NoiselessSCMRecourse; it removed the free pass that was concealing the
...
not hold, and section 7.4 ("PearlSCMRecourse validity collapses at full scale") is not a separate
...
PearlSCMRecourse and CftsCels on `full`), D11 (`table_axis_c_cf_faith.csv` carries 18 duplicate rows,
...
full-scale reachability gate remain removed. The NoiselessSCMRecourse/PearlSCMRecourse implementation stage has been
...
**Phase A — fix recourse and metric semantics (stages 1-3).** Repair NoiselessSCMRecourse/PearlSCMRecourse candidate
...
|---|---|---|---|
| NoiselessSCMRecourse intervention delta, `full_nl` | 6.4e-08 | > `INTERVENTION_TOL` on >= 90% | Below tolerance there is no action |
| NoiselessSCMRecourse `frac_vacuous`, `full_nl` | 1.00 | <= 0.20 | Reject the noiseless-continuation free pass |
| PearlSCMRecourse `frac_degenerate`, `full_nl` | 1.00 | <= 0.10 | Restore CF-faith scorable output |
| NoiselessSCMRecourse classifier `validity`, `full_nl` | 0.000 | > 0.30 | Classifier outcome remains the standard validity metric |
| NoiselessSCMRecourse `frac_no_cf_found`, `full_nl` | absent | <= 0.20 | Make optimizer failure explicit without redefining validity |
...
- `causaltemp_xai/methods/counterfactual/scm_recourse.py` -- candidate tie-break, bounded proximity
...
| 5 | [Local full validation](stages/05-local-full-validation.md) | DONE | All three smoke configs completed the seven-method pipeline; C1-C18 pass at smoke scope; fallback overhead is 9.9%, so the 30-hour jobs remain unchanged; 614 passed, 1 xfailed. C19-C20 remain correctly deferred to Stage 7. | `32e6f7a` + `6058400` + this commit |
...
| 3 | `cf_faith_*` cannot rank (D3) | planning | med | Requires redesigning the metric, not fixing a bug. `rollout_hard` equals 1.0 iff the method is NoiselessSCMRecourse or OracleCF-Rollout, on all 5 datasets; the soft variant ties the oracle to 4.6e-07. | Present it as a semantics-admission gate, never as a ranking column. A genuinely graded faithfulness metric is a research task. | OPEN |
...
| 6 | `scm_noise_plausibility` has no ceiling (D9) | planning | low | Small but needs a design call. `axis_c.py:253-280` documents 1.0 as the Pearl oracle's value; measured 0.87-0.98, and PearlSCMRecourse (0.9779) and CftsCels (0.9777) beat the oracle (0.9749) on `full`. Likely benign finite-sample bias in `mean|eps|`. | Add a `Factual` pseudo-method row so the metric's practical ceiling is measured, not assumed. ~20 LoC in phase 05. | OPEN |
...
**2026-08-21 — Reduced scope with NoiselessSCMRecourse fix restored.** The user removed the errata/repository-
hygiene and reachability stages, then restored the NoiselessSCMRecourse/PearlSCMRecourse implementation stage. The
...
**2026-08-21 — Rename project-owned positive controls.** The live class names, registry keys,
CLI method labels, filenames, and run-2 table values are `NoiselessSCMRecourse` and
`PearlSCMRecourse`. The former `CARLARecourse`/`PearlCARLARecourse` APIs and
`CARLA`/`PearlCARLA` schema labels are removed rather than aliased. Run-1 result trees and tables
remain byte-stable historical artifacts, documented by a naming note in the run-1 report.

**2026-08-21 — Prediction-only fallback replaces six halvings.** Stage-1 execution measured
0/3 NoiselessSCMRecourse and 0/3 PearlSCMRecourse flips after six successive `lam_prox` halvings on regenerated
...
NoiselessSCMRecourse took 65.0 s and PearlSCMRecourse 103.9 s locally. Both reached validity 1.00 with no-CF,
...
Pearl-scorable fraction, as `full_nl/NoiselessSCMRecourse` demonstrates.
