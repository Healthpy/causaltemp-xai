...
|---|-------|--------|-------|--------|
| 1 | [Environment & packaging fix](stages/01-env-and-packaging.md) | IN_PROGRESS | Infra DONE (`592654a`): uv env (py3.13, torch 2.12), build-backend fixed, CI→uv, requirements→pointer, cf_faith docstring fixed, CF-faith #1 locked → 21/21 green. **Pending (step 7):** add `pearl_delta` metric alongside #1 + `TestCFFaithPearl` → 2nd commit | `chore(infra): … baseline` (done) · `feat(metrics): dual CF-faith` (pending) |
...
- **2026-05-30 [BLOCKING — metric semantics, entangled with Stage 4]** With the syntax fixed, `test_identical_cf_hard_is_one` fails: an identical CF (`x_cf == x_orig`, null intervention) scores `hard=0.0`, the test asserts `1.0`. Root cause is a genuine design fork in what the forward-sim residual compares against:
  ...
  - **EXTENDED (user decision, 2026-05-30):** rather than discard #3, **keep both metrics** — add Pearl delta-recursion as `CFfaith(semantics="pearl_delta")` alongside the default. Planned in Stage 1 (step 7); implementation is a pending follow-up commit. See Decisions for the downstream both-metric reporting contract. This converts the Stage 6 OOD-plausibility "watch item" into an explicitly-measured rollout-vs-pearl contrast.
