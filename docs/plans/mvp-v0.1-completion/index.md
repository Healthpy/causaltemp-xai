...
| R8 | Stale docstrings / minor dup: `cf_faith.py:39-43` class docstring pre-dates the dual-metric extension; `tcn.py:438` `predict` re-derives single/batch detection already in `_as_batch_tensor`. | P3 | `metrics/cf_faith.py`, `classifiers/tcn.py` | Cosmetic. | Refresh docstring; dedup the layout check. |
