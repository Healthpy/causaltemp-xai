...
The repo (`docs/mvp_plan.md`, `docs/general_plan.md`) targets a 4-week MVP: one synthetic SCM benchmark, a LSTM classifier, 3 CF methods, Axis-C + Shift-VR-lite metrics, and 3 figures showing validity ranks ≠ CF-faith ranks (hypotheses H1, H3, H4-partial).
...
| LSTM module | `classifiers/lstm.py` | ◑ `LSTM` + `train_lstm()` exist; no val/test split, no checkpoint, no predict/proba interface, no tests |
...
| Experiment harness | `experiments/run_all.py` | ❌ written against a non-existent API (`n_vars/lag`, `sample_with_labels`, `LSTMClassifier`, `cf_faith_hard/soft`, `CARLACF`, `generate_batch`) — does not import |
...
- **Phase A — Foundation (Stages 1–3):** uv environment + packaging fix + green baseline; lock the benchmark config (two-tier smoke/full) and persist datasets; add a sklearn-style `LSTMClassifier` wrapper trained to >90% with a frozen checkpoint.
...
| LSTM >90% test accuracy, frozen checkpoint | `train_lstm` only, no eval/checkpoint | ≥0.90 (≥0.85 documented fallback) test acc; checkpoint on disk | Stage 3 |
...
- `causaltemp_xai/classifiers/lstm.py` + `classifiers/__init__.py` — add `LSTMClassifier` wrapper
...
| 3 | [LSTMClassifier wrapper & training](stages/03-classifier-and-training.md) | DONE | `LSTMClassifier` wrapper (fit w/ val early-stop + best-val restore, predict/predict_proba, differentiable `torch_logits`, save/load, `(T,k)`/`(N,T,k)` shape contract w/ internal permute); CLI w/ tuning knobs trains+freezes `data/linearscm_t/<cfg>/lstm.pt`. 8 new classifier tests (overfit, shapes, save/load, grad-flow, shape-contract w/ CFfaith). **full test_acc≈0.79 — below 0.90/0.85 target; user-accepted (avg-pool vs endpoint-label cap), documented in Issues.** 43/43 green. | `77a0a52` |
...
- **2026-05-30 [Stage 3 — classifier accuracy below target, user-accepted]** The locked `full` LSTM reaches only **~0.79 test accuracy** (train 0.831 / val 0.784 / test 0.787), **below the 0.90 target and the 0.85 documented fallback**. Root cause is an architecture/label mismatch, **not** under-training: the binary label is a threshold on the **final-timestep** value of variable 0, but the scaffold `LSTM.forward` uses **global average pooling** over all T timesteps (`features.mean(dim=-1)`), which dilutes the endpoint signal. Empirically confirmed across three runs — avg-pooling caps generalization at ~0.75–0.79 and *overfits* when pushed harder (lr=3e-3 → train 0.74 but test 0.67; val-loss turns up ~epoch 60 while train-loss keeps falling). The label is fully deterministic given the trajectory and the SCM is Markov-1, so a **last-timestep-pooled** LSTM would essentially solve it (>0.95) — the pool is the sole bottleneck.
  - **RESOLVED (user decision, 2026-05-30):** **Document ~0.79 honestly** — keep the scaffold's global-average-pooling `LSTM` unchanged (do not alter the architecture or the locked label), freeze the best-val-restored checkpoint, and record the gap here. CF-faith is classifier-agnostic, so H1/H3 (the validity-vs-CF-faith contrast) remain meaningful; validity/proximity sit on a moderate (~0.79) classifier, which is an acknowledged limitation, not a bug. Rejected: (a) switching to last-timestep pooling (would breach "keep the LSTM module"), (b) redefining the label (would alter the locked Stage 2 ground truth). An optional `pooling='last'` knob / stronger classifier is candidate **v1.0** future work.
  - **Checkpoint provenance:** `data/linearscm_t/full/lstm.pt` is **gitignored** (regenerate-in-CI policy). Reproduce with: `uv run python -m causaltemp_xai.classifiers.lstm --config full --train --patience 20` (defaults: lr=1e-3, n_levels=4, n_channels=64, dropout=0.2, max_epochs=100, seed=42). `fit()` restores the lowest-val-loss epoch, so the frozen weights are the best-generalizing point, not the over-fit tail.
...
- **2026-05-30** (pre-review) — Datasets are **regenerated in CI** (deterministic via seed), never committed. Shape contract standardised on `(T,k)` at public boundaries (permute lives only inside `LSTMClassifier`).
...
All 8 stages (17 commits) were reviewed after completion via four parallel feature-group subagents (Foundation 1–3 · CF Methods 4 · Metrics/Eval+Attribution 5–6 · Robustness+Synthesis 7–8). **Every group returned PASS. No P0/P1 (correctness/architectural) defects. Full suite 82/82 green (independently re-confirmed).** The bit-identical-mechanisms claim (Stage 7) and the IG completeness axiom (Stage 6) were verified in code, not just asserted. The findings below are **hardening/polish deferred to the v1.0 backlog** — none block the v0.1 freeze or the open MR.

### Backlog (deferred — v1.0 hardening)

| ID | Item | Sev | File | Why deferred / status | Suggested fix |
|----|------|-----|------|-----------------------|---------------|
| R1 | `intervention_t == T-1` edge: a final-timestep-only CF scores `hard=1` (empty forward loop → residual over a single fixed point). The exact silent-H1-inversion trap. | P2 | `metrics/cf_faith.py:108` | Not live — *handled downstream* by the Stage 5 fail-fast (Wachter `t0` clusters at 0, verified on full run). The metric itself has no guard. | Add a docstring warning, or `if intervention_t >= T-1:` return/warn so a future caller can't trip it unnoticed. |
| R2 | `target_acc` early-stop is silently inert whenever a val set is passed (val-loss patience is then the only stop). | P2 | `classifiers/lstm.py` (`fit`) | Not live — the training CLI always passes a val set and does not expose `--target-acc`. | Document that `target_acc` applies only when `not has_val`, or honour it alongside val-loss. |
| R3 | "rollout-hard=1 by construction" is *conditionally* guaranteed on `derive_intervention_t(x,cf)==t0`. | P3 (flagged P2; on analysis not realizable) | `methods/carla.py:153` | Verified **not realizable** under current tols: `derive.tol=1e-6` ≪ first rolled-out timestep's ~noise-std difference, so `derive` never skips past `t0+1`; the retro window `[:t]` never reaches rolled-out rows; the noiseless-rollout property holds for any sub-window → `hard=1` preserved even if `t0` is detected one step late. | Add a defensive `assert derive_intervention_t(x,cf)==t0` to harden against future tolerance changes. |
| R4 | `shift_vr` NaN sentinel serialized as bare `NaN` — invalid strict JSON (Python `json.load` and `figures.py` tolerate it; external/JS parsers reject). | P3 | `experiments/run_all.py` → `results.json:89` | Internal consumer handles it. | Replace `float('nan')` with `None` (→ JSON `null`) before `json.dump`. |
| R5 | `stratified_split` could yield an empty val/test split for a class with 1–2 samples. | P3 | `data_io.py:65-69` | Not reachable by the locked configs (N=500/10000, balanced). | Guard or document the minimum per-class size. |
| R6 | Misleading comments / loose handling: Wachter `best_cf` comment says "keep the closest attempt" but keeps the last (highest-λ) round; CARLA collapses a 2D actionable mask to `m[-1]` rather than `m[t0]`. | P3 | `methods/wachter.py:103,121`, `methods/carla.py:147-151` | Behaviour is sane (benign for the all-actionable default). | Reword the Wachter comment; index the 2D mask at `m[t0]` or document time-invariance. |
| R7 | Minor figure inconsistency: fig1 plots `cf_faith_rollout_soft`, fig3 uses `..._hard`. | P3 | `experiments/figures.py:73` | Defensible (per-instance hard is degenerate {0,1}); docstring is honest. | Align on one variant or add a one-line comment. |
| R8 | Stale docstrings / minor dup: `cf_faith.py:39-43` class docstring pre-dates the dual-metric extension; `lstm.py:438` `predict` re-derives single/batch detection already in `_as_batch_tensor`. | P3 | `metrics/cf_faith.py`, `classifiers/lstm.py` | Cosmetic. | Refresh docstring; dedup the layout check. |

**Note (not a finding):** CI triggers on `pull_request: branches:[main]`, so the open MR runs CI; only push-to-feature-branch is untriggered (irrelevant for the MR). CI pins Python 3.11 while the local/locked env is 3.13 — a minor version-skew exposure, no observed failure.
