...
| 2 | [MLPMechanism implementation](stages/02-mlp-mechanism.md) | DONE | `MLPMechanism` added: per-node batched 2-layer MLP, additive-noise contractive transition (decay + gain·tanh), spectral-norm-capped weights, zero-parent guard. numpy/torch parity <1e-5; 300-step rollout bounded over 10 seeds; nonlinearity + masking + npz round-trip tested. Full suite green (105 tests). | `c8492bf` |
| 3 | [NlinearSCMT generator](stages/03-nlinear-generator.md) | DONE | `NlinearSCMT` added: shares graph sampler with linear via factored `_sample_lag_mask` (golden L1/L2 still pass — RNG order frozen), builds MLP mechanism in `__init__` before noise (Axis-D), median-threshold labels, deterministic divergence-resample + clip fallback. 19 new tests (shapes, boundedness over T=200×20 seeds, sparsity, data-level nonlinearity, forced-divergence reproducibility, KS). Full suite green (124 tests). | `78647fc` |
| 4 | [CF-faith generalization + oracle structural-CF](stages/04-cf-faith-and-oracle.md) | DONE | `pearl_delta` reformulated as abduction-action-prediction (`cf_faith.py`, shared `_window` helper); golden L1 **still bit-for-bit**, L2 unchanged. New `benchmark/structural_cf.py` (oracle CF + `abduct_noise`, noisy & noiseless variants). 7 structural_cf tests (abduction <1e-6, oracle positive control, mutual exclusivity, retro/random negatives) + 4 nonlinear cf_faith cases. Full suite green (135 tests). | `77d55ea` |
| 5 | [Config presets + nonlinear persistence](stages/05-config-and-persistence.md) | DONE | `BenchmarkConfig` gains `mechanism_type`/`nonlinear` (defaults keep linear byte-identical); `SMOKE_NL`/`FULL_NL` registered; `shifted_config` preserves both (Axis-D). `data_io` dispatches via new `build_generator`, persists MLP via `state_dict`, records `nonlinear` in `meta.json`; `DEFAULT_OUT_DIR`→`data/scm_t` (keyed by unique name), `.gitignore` now ignores `data/`. 9 new tests (dispatch, Axis-D, save/load round-trip, forward bit-identical). Full suite green (144 tests); CLI `--config smoke_nl` round-trips. | `93097d7` |
| 6 | [Docs, experiments wiring, future-work notes](stages/06-docs-and-experiments.md) | DONE | `run_all.py` routes nonlinear configs (`smoke_nl`/`full_nl`) to a new `run_nonlinear` oracle-CF path — no checkpoint / no real CF methods (Backlog #2), CF-faith scored on the Stage-4 oracle (Pearl→pearl_hard=1, noiseless→rollout_hard=1). `--config smoke_nl` runs end-to-end, writes `results.json`+`per_instance.csv` (verified). Docs updated: README NlinearSCM-T section + stale-contract fixes (`mechanism`/`data/scm_t`), `general_plan.md` Benchmarks #2 + Scope-Boundaries, scope notes in `generator.py`/`mechanisms.py` docstrings. Full suite green (144). | `4ae6adb` |
...
- **D10 — Shared graph sampler via factored per-lag mask (executor, 2026-06-21).** Stage 3 calls for a shared `_sample_graph(k, L, sparsity, rng)`. A naïve extraction that pre-draws all lag masks then draws all linear coefficients would reorder `LinearSCMT`'s RNG sequence (currently interleaved mask→coef per lag) and break the golden L2 `X_sum` (pinned to `rtol=1e-9`). Resolution: factor the atomic per-lag Bernoulli draw into `_sample_lag_mask(k, lag, sparsity, rng)` (with the no-self-loop-at-lag-1 rule); `LinearSCMT` calls it *inside* its per-lag loop (interleaving preserved, golden bit-for-bit), and `_sample_graph` loops it for `NlinearSCMT`. Both share the rule and the draw without un-freezing v0.1.
- **D11 — Divergence handling = deterministic resample then clip fallback (executor, 2026-06-21).** `NlinearSCMT.generate` rolls all trajectories, then up to `max_resample=10` rounds resamples *only* the diverging rows (`max|x| > clip=1e3` or non-finite) by redrawing their init+innovation noise from the seeded RNG in **sorted-index** order (wall-clock-independent ⇒ reproducible). Any trajectory still diverging after the budget is clipped to `±clip`, guaranteeing finite/bounded output. The contractive Stage-2 design means this path almost never fires on real configs; a forced-divergence test (monkeypatched high-gain mechanism + `clip=1`) exercises the fallback and asserts finite + reproducible.
- **D13 — Nonlinear harness path uses the oracle CF as the in-house "method" (executor, 2026-06-21).** Stage 6 wires `experiments/run_all.py` for nonlinear configs by branching in `run()` on `cfg.mechanism_type != "linear"` to a dedicated `run_nonlinear()`. That path is intentionally *not* the linear harness: it needs **no LSTM checkpoint** and runs **no real CF methods** (Wachter/DiCE/CARLA on the nonlinear SCM = Backlog #2), so it auto-generates the dataset if absent and scores CF-faith **classifier-free** on the Stage-4 oracle structural-CF as a built-in positive control. Two mutually-exclusive oracle variants are emitted per the rollout-vs-pearl contract — `OracleCF-Pearl` (noisy ⇒ `pearl_hard=1`) and `OracleCF-Rollout` (noiseless ⇒ `rollout_hard=1`) — using the deterministic intervention `do(x[T//2, i%k] = x[…]+1.5)`. `results.json` keeps the linear schema (`provenance`+`methods`) with `validity`/`ood` set to `null` (classifier-agnostic, documented in the provenance `note`). IG/Shift-VR are omitted on the nonlinear path (both need a classifier + CF methods).
- **D12 — Pearl reformulation is bit-for-bit backward-compatible (executor, 2026-06-21).** Stage 4 replaced the homogeneous-delta `pearl_delta` rollout with full abduction-action-prediction (abduct `eps` from the factual, roll `x_pred` reusing `eps`, compare `x_cf` vs `x_pred`). Mathematically this is a strict generalization that reduces to `delta[t]=sum_l A_l@delta[t-l]` for linear `f`; the float ops differ (the old path runs `f` directly on the delta, the new path runs `f` on `x_orig`/`x_pred` separately), so a ULP-level drift on the L1 golden `soft` was the risk. **Empirically it did not move**: `test_golden_linear.py` L1 still passes bit-for-bit (`==`) and L2 within its existing rtol — no special-casing of `LinearMechanism` was needed, so cf_faith stays a single unified code path. The shared `_window` builder is duplicated in `metrics/cf_faith.py` and `benchmark/structural_cf.py` (both private) to avoid a cross-package import; they encode the identical oldest→newest zero-pad window contract.
