...
- **Axis C:** validity, proximity (L1/L2), sparsity, OOD (IsolationForest), TRSI (`metrics/axis_c.py`; tested). *(2026-07-15: `ivr` and `proximity_dtw` removed — IVR was 0-by-construction in the live wiring and subsumed by CF-faith's retro gate; the DTW helper was unwired. TRSI reframed as an adopted, mechanism-free proxy, not a faithfulness criterion. See `docs/spec_code_reconciliation.md` §3–§4.)*
...
- **Definition divergences (RECONCILED 2026-07-14 — see `docs/spec_code_reconciliation.md`):** the plan defined CF-faith as normalized DTW, ICC as latent-traversal, and Eq. 1 via the operator dictionary; the wired code implements residual/two-semantics CF-faith, attribution-mass ICC (with the latent-traversal `icc_latent` present but dormant), and additive-noise MLP mechanisms (with the operator dictionary present but dormant in `scm/operators.py`). Resolved to one source of truth per divergence (the live code) and `updated_general_plan.md` edited to match. Two P1 follow-ups remain: wire `icc_latent`; optionally add DTW-to-oracle as a complementary CF-faith number.
...
16. ~~**[PI + M&C]** Reconcile plan-vs-code divergences: CF-faith (DTW vs residual), ICC (latent flip-rate vs attribution mass), mechanism family (operator dictionary vs MLP).~~ **DONE 2026-07-14** — resolved to the live code as source of truth; `docs/spec_code_reconciliation.md` records the decisions and `updated_general_plan.md` now describes the implementation. Follow-ups: wire `icc_latent` (P1), optional DTW complementary metric (item 26).
...
| R10 | **Spec–code divergence** — plan math (operator dictionary, DTW CF-faith, latent ICC) ≠ shipped code | ~~Certain today~~ **RECONCILED 2026-07-14** | Major | Resolved: `docs/spec_code_reconciliation.md` + `updated_general_plan.md` edits describe the live code (item 16 done; 2 P1 follow-ups noted) |
