...
> **RESOLVED 2026-07-30 — the predicted problem is real, but this proposed fix is wrong.**
> Full-scale diagnosis (`DECISIONS.md`) confirms the long-horizon collapse but rules out
> `lam_prox` tuning (and any delta-bound change) as the remedy: on `full_nl` a **unit**
> intervention at `t0=25` survives to the label-determining final timestep as **exactly
> 0.0** (float32 underflow; `1.2e-07` at `t0=50`). The effect has decayed below machine
> precision, so no reweighting recovers it — the measured classifier gradient
> (`max|grad|=2.97e-09` at step 0) is a *correct* reading of zero causal effect, not an
> optimisation failure.
>
> The sharper framing: `t0_fractions` is a fraction of `T`, but the decay is exponential
> in the **absolute** horizon, so fractions validated at `T=30` (horizon 15, validity
> 1.00 above) become structurally hopeless at `T=100` (horizon 50–75). A `full_nl` t0
> sweep (n=15) gives validity 0.00 at `t0=0.25/0.5/0.75/0.9`, recovering only to 0.60 at
> `t0=0.95`, with required `max|delta|` growing `8.3e-07 → 4.7`. PI decision: config left
> unchanged; PearlCARLA's validity=0 is reported as a horizon limit rather than patched.
>
> Also found, and more consequential — see the H3b caveat in `DECISIONS.md`: §2.3's
> suspicion about "CARLA's apparent long-horizon advantage" is confirmed and quantified.
> CARLA's intervention decays identically (`2.6e-04` on `full`), but its **noiseless**
> rollout discards the abducted noise, displacing the trajectory by ~0.67 at `T`
> *regardless of the intervention* — so its validity=1.00 comes from noise-stripping, not
> from effective recourse.
