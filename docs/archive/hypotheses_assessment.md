...
> ⚠️ **OPEN CAVEAT ON THIS VERDICT (added 2026-07-30, `DECISIONS.md`).** Two
> problems, both pending this document's rewrite against current LSTM results:
>
> 1. **The validity numbers above are stale and now contradicted.** This
>    section reports CARLA validity collapsing to 0.00; the current
>    post-fix LSTM run gives CARLA **1.00** on `full` and 0.00 on `full_nl`.
>    Do not cite either figure until the rewrite.
> 2. **"Faithful by construction" may be doing less work than claimed.**
>    CARLA's *intervention* decays to nothing at these horizons (2.6e-04 on
>    `full`, 1.1e-10 on `full_nl`) — yet it still scores validity 1.00 on
>    `full`, because its **noiseless rollout discards the abducted noise**,
>    displacing the trajectory ~0.67 at `T` regardless of the intervention.
>    So part of both its validity *and* its rollout-faithfulness follows from
>    the semantics rather than from effective recourse. Until this is
>    quantified, **H3b must not be cited as independent evidence** that
>    graph-aware recourse achieves faithfulness — it is closer to a
>    tautology of the rollout definition than a result. PearlCARLA, which
>    correctly re-injects noise, has no such crutch and posts validity 0.00.
