...
\end{equation}
The world label is $y_g(\bx)=\mathbb{1}\{g(\bx)>\theta\}$, whose final dependency is $t_{\mathrm{label}}$; $h:\R^{T\times k}\to\{0,1\}$ denotes the evaluated classifier's hard decision, and $z_h:\R^{T\times k}\to\R^2$ denotes its logits.
...
\begin{equation}
  \lambda_{\mathrm{pred}}\,\mathrm{CE}\bigl(z_h(\hat{\bx}),\,y^\star\bigr)
