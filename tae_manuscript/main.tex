...
Designing the benchmark surfaced a construct-validity issue that is part of the contribution rather than an implementation detail. Its answer depends on how a proposal is \emph{read}: interpreting a counterfactual as a single $\dop()$ at one timestep tests whether one action generates the edited suffix, while interpreting every edited slice as an intervention tests the full action schedule. We register both endpoint readings as a sensitivity analysis and report \textbf{do-complexity} $D$, the number of timesteps that the schedule must impose before the mechanism reproduces the proposal. A proposal with $D$ close to the trajectory length is a trajectory rewrite under this benchmark; agreement obtained after declaring nearly every slice an action leaves little for the mechanism to predict.

A second finding identifies the distance that governs temporal recourse. Stationarity of a linear VAR requires a spectral radius below one \citep{lutkepohl2005}, so an intervention's effect on the label attenuates geometrically in the distance from the intervention time to the \emph{label site}, not to the end of the trajectory. Those two distances coincide when the label is terminal, as in LinearSCM-T, NlinearSCM-T, SpringSCM-T, and the spring experiments of \citet{bahri2025causalfeasibility}. They differ in whole-series classification when the discriminative site is interior. Recourse feasibility is therefore a property of both the benchmark configuration and the method. Every temporal recourse evaluation must report the intervention-to-label distance with validity.
...
\begin{enumerate}
  ...
  \item \textbf{Intervention-to-outcome distance as a required reporting condition.} We derive attenuation in $t_{\mathrm{label}}-t_0$ and show that $T-t_0$ is valid only when the label is terminal.
  \item \textbf{A benchmark suite with exact structural counterfactuals}: three additive-noise temporal SCM families shipping their recorded graph or coupling structure, complete transition mechanism, and oracle abduction--action--prediction counterfactuals, with the label functional exposed explicitly rather than hidden in the construction.
...
The paired evaluation reveals a systematic gap between conventional model-relative scores and structural replay on the same proposals.
...
COMTE and scorable CONFETTI proposals have $D=100$; Wachter has $98.3,100,100$. They are near-complete schedules under Pearl semantics.  M-CELS has $D=6.7,26.4,55.3$. Under the registered threshold perturbation, its values change $6.1$--$16.5\times$ but remain ordinally less complete than the other baselines. Its claimed rate still exceeds replay in every family.  This prevents a misleading interpretation of low replay error under a full schedule: if every timestep is declared an action, reproducing the proposal says little about causal propagation.  COMTE and CONFETTI illustrate this vacuity directly. For M-CELS, $D$ supports the schedule-density comparison when reported with the changed-coordinate tolerance.
...
The benchmark requires the written $G,F$ to be the additive-noise data-generating SCM, with positive lags, recoverable factual noise, and no unmeasured confounding. The presets also fix $L=1$. These conditions enable exact unit-level replay. They exclude latent confounding, measurement error, learned mechanisms, non-invertible noise, and misspecified structural equations.

The benchmark reconstructs intervention schedules from changed coordinates using a shared $10^{-3}$ tolerance. It evaluates actions expressed in the observed variables and structural equations. A deployment domain can instead define a dense trajectory as one high-level action. The two registered endpoint readings expose the cost of this choice, while the M-CELS sensitivity result requires reporting the tolerance with $D$.

All three families are synthetic and use a terminal threshold label. The experiments therefore leave interior and distributed label sites for future evaluation. The study also uses one generated benchmark and one classifier fit per family. Multi-seed experiments are needed to estimate uncertainty in method comparisons and variation across SCM draws. The present paired results establish the conventional--structural measurement gap under the reported benchmark realizations.
