...
\brief{CauseMe-style factors, not a file dump: mechanism family (VAR /
per-node MLP / Newtonian springs), contraction vs.\ near-conservative
dynamics, horizon $T$, label site, intervention time $t_0$. Additive noise
is load-bearing (keeps abduction exact). Label functional is a
configurable axis so the reporting rule is testable. Axis~A diagnostics
(SHD, lag F1, graph-error) characterise the benchmark once, never printed
as per-method scores.}

\subsection{Tiers and what each can support}
\label{sec:setup-tiers}

\brief{Internal vs.\ external validity (Liao/Raji), not a realism ladder.
Tier~1 (synthetic, written $F$): every headline result and every
metric-validity argument. Tier~2 (real signal, no graph): whether the
horizon finding travels. Tier~3 (domain DAG) deferred; do not promise
SepsisSim. Discovered-graph CF-faith on real data: variance quantified,
bias disclosed, not resolved.}

\subsection{Methods, oracles, and negative controls}
\label{sec:setup-methods}

\brief{Standard temporal CFs (Wachter, COMTE, CONFETI, CELS). Positive
controls, labelled as such: PearlSCMRecourse (single $\dop()$, $D=1$) and
NoiselessSCMRecourse (emits the rollout it is scored against). Foil:
TSCausalCF (Bahri; faithfulness not true by construction). Negative
controls in the main figure: vacuous rewrite ($D\approx T$) must not earn
causal credit. Classifier fixed (LSTM); method identity, seed, and
config are provenance, not afterthoughts.}
