...
\end{equation}
The world label is $y_g(\bx)=\mathbb{1}\{g(\bx)>\theta\}$, whose final dependency is $t_{\mathrm{label}}$; $h:\R^{T\times k}\to\{0,1\}$ denotes the evaluated classifier's hard decision, and $\mathbf{z}_h:\R^{T\times k}\to\R^2$ denotes its logits.
