"""
TSCM forward simulator — Plan Eq. 1.

X_t^(j) := sum_{X_{t-tau}^(i) in pa(X_t^(j))} w_ij * phi_ij(X_{t-tau}^(i)) + U_t^(j)

The simulator accepts pre-sampled exogenous noise U (shape: N x T x M) so that
...
def simulate_tscm(
    ...
    noise_seq : np.ndarray
        Shape (N, T_total, M) where T_total = T + burn_in.
    ...
    -------
    np.ndarray
        Shape (N, T, M) — observed multivariate time series after burn-in.
    """
    N, T_total, M = noise_seq.shape
    T = T_total - burn_in
    ...
    assert result.shape == (N, T, M), f"Shape mismatch: {result.shape}"
    return result
...
def sample_noise(
    ...
    if distribution == "laplace":
        return rng.laplace(loc=0.0, scale=scale, size=(N, T_total, M))
    elif distribution == "uniform":
        return rng.uniform(-scale * np.sqrt(3), scale * np.sqrt(3), size=(N, T_total, M))
    elif distribution == "gaussian":
        # Gaussian ablation: breaks non-Gaussian identifiability assumption (used in H5)
        return rng.normal(0.0, scale, size=(N, T_total, M))
    else:
