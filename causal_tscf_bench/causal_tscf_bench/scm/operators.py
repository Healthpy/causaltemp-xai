"""
Invertible operator dictionary Phi for TSCM mechanism sampling (Plan Eq. 1).

Each operator phi_ij is applied element-wise to a parent channel's lagged value.
All operators in INVERTIBLE_OPERATORS admit exact noise abduction (Plan Eq. 2).
The "step" operator is non-invertible and is reserved for the adversarial
non-monotonic ablation of NlinearSCM-T only.
